...
const ProductCard: React.FC<ProductCardProps> = ({ product, rank }) => {
    ...
    return (
        <div className="group relative w-full bg-white rounded-xl sm:rounded-3xl overflow-hidden border border-[#EBEBEB] shadow-[0_2px_8px_rgba(0,0,0,0.03)] hover:shadow-[0_16px_40px_rgba(0,0,0,0.12)] hover:border-[#1769FF]/25 transition-all duration-300 ease-out flex flex-col justify-between">
            ...
            {/* Image Container — aspect-square optimisé pour 360px */}
            <div className="relative w-full aspect-square bg-[#F7F9FC] flex items-center justify-center border-b border-[#EBEBEB]/60 overflow-hidden">
                ...
                <button
                    ...
                    className={`absolute top-2 right-2 z-20 size-7 sm:size-10 rounded-full flex items-center justify-center transition-all duration-200 cursor-pointer shadow-xs ${
                        isWishlisted
                ...
                {/* Product Image */}
                <Link href={`/product/${productId}`} className="relative w-full h-full block p-1.5 sm:p-3">
                    {imgError ? (
            ...
            {/* Content Area — Padded compactly for 360px grid */}
            <div className="p-2 sm:p-5 flex flex-col flex-1 justify-between gap-1.5 sm:gap-3 bg-white">

                <div className="space-y-0.5 sm:space-y-1">
                    {/* Category & Rating */}
                    <div className="flex items-center justify-between gap-1">
                        <span className="text-[9px] sm:text-[12px] font-extrabold text-[#1769FF] uppercase tracking-wider truncate">
                            {product?.category || 'High-Tech'}
                        </span>
                        <div className="flex items-center gap-0.5 shrink-0">
                            <Star size={10} fill="#F59E0B" className="text-amber-400 sm:hidden" />
                            <Star size={13} fill="#F59E0B" className="text-amber-400 hidden sm:block" />
                            <span className="text-[10px] sm:text-[13px] font-extrabold text-[#101828]">{avgRating}</span>
                        </div>
                    </div>

                    {/* Product Name */}
                    <Link href={`/product/${productId}`} className="block">
                        <h3 className="text-[11px] sm:text-[15px] font-bold text-[#101828] line-clamp-2 leading-tight group-hover:text-[#1769FF] transition-colors">
                ...
                <div className="space-y-1.5 sm:space-y-2.5">
                    ...
                    <div className="flex items-baseline justify-between gap-1">
                        <span className="text-[12px] xs:text-[13px] sm:text-[18px] font-black text-[#101828] leading-none">
                            {formatPrice(price)}
                        </span>
                        {isOutOfStock && (
                    ...
                    <button
                        ...
                        className={`w-full font-bold text-[10px] sm:text-sm py-2 sm:py-3 px-2 rounded-lg sm:rounded-2xl transition-all duration-200 flex items-center justify-center gap-1 active:scale-95 cursor-pointer ${
                            isOutOfStock
                    ...
                    >
                        {isOutOfStock ? (
                            ...
                            <><Check size={12} /><span>Ajouté</span></>
                        ) : (
                            <><ShoppingCart size={12} /><span>Ajouter</span></>
                        )}
                    </button>
                </div>
