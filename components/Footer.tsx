...
const Footer: React.FC = () => {
    ...
    const linkSections = [
        {
            title: "PRODUITS",
            links: [
                { text: "Écouteurs", path: '/shop?search=Earbuds', icon: null },
                { text: "Casques", path: '/shop?search=Headphones', icon: null },
                { text: "Smartphones", path: '/shop?search=Smartphones', icon: null },
                { text: "Ordinateurs portables", path: '/shop?search=Laptops', icon: null },
            ]
        },
        {
            title: "NAVIGATION",
            links: [
                ...
                { text: "Politique de confidentialité", path: '/pricing', icon: null },
                { text: "Devenir membre Plus", path: '/pricing', icon: null },
                { text: "Créer votre boutique", path: '/create-store', icon: null },
            ]
        },
        {
            title: "CONTACT",
            links: [
                { text: siteSettings?.phone || "+1-212-456-7890", path: '/contact', icon: PhoneIcon },
                { text: siteSettings?.email || "contact@sentechplus.com", path: '/contact', icon: MailIcon },
                { text: siteSettings?.address || "794 Francisco, 94102", path: '/contact', icon: MapPinIcon }
            ]
        }
    ];
    ...
    return (
        <footer className="bg-slate-50 border-t border-slate-200 mt-20 pt-16">
            <div className="max-w-7xl mx-auto px-6">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-10 lg:gap-16 pb-12">
                    
                    {/* Brand Info */}
                    <div className="md:col-span-1">
                        <Link href="/" className="inline-block mb-4 transition-transform hover:scale-[1.02]">
                            <Logo />
                        </Link>
                        <p className="text-slate-500 text-sm leading-relaxed max-w-sm">Bienvenue chez SenTech Plus, votre destination ultime pour les appareils intelligents et accessoires high-tech. Des smartphones aux montres connectées en passant par les accessoires essentiels, nous vous apportons le meilleur de l'innovation.</p>
                        <div className="flex items-center gap-3 mt-5">
                            {socialIcons.map((item, i) => (
                                <Link 
                                    ...
                                    className="flex items-center justify-center size-10 bg-white hover:bg-blue-50 border border-slate-200 hover:border-blue-200 hover:scale-105 transition-all rounded-full shadow-2xs"
                                >
                                    <item.icon />
                                </Link>
                            ))}
                    ...
                    <div className="md:col-span-3 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 md:gap-12 text-sm">
                ...
                <p className="py-4 text-sm text-slate-500">
                    Copyright 2025 © SenTech Plus Tous droits réservés.
                </p>
            </div>
        </footer>
    );
